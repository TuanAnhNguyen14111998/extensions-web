...
//-----------------------------------------------------------------------------
    ...
    // mot doi tuong bieu dien mot vi tri trong he truc toa do hai chieu Oxy
    ...
    // phuong thuc khoi tao Tilemap
    function Tilemap() {
        // arguments la mot mang cac tham so duoc truyen vao.
        // ham initialize se duoc dinh nghia phia sau
        this.initialize.apply(this, arguments);
    }

    // tao doi tuong Tilemap ke thua lop PIXI.Container
    Tilemap.prototype = Object.create(PIXI.Container.prototype);

    // constructor cho Tilemap
    Tilemap.prototype.constructor = Tilemap;

    // khoi tao cho Tilemap
    Tilemap.prototype.initialize = function() {
        PIXI.Container.call(this);

        // cac thong so cho Tilemap:
        // margin
        this._margin = 20;
        // chieu rong
        this._width = Graphics.width + this._margin * 2; // 340
        // chieu cao
        this._height = Graphics.height + this._margin * 2; // 340
        // chieu rong cho tile con
        this._tileWidth = 48;
        // chieu cao cho tile con
        this._tileHeight = 48;
        // chieu rong cho ca map
        this._mapWidth = 0;
        // chieu cao cho ca map
        this._mapHeight = 0;
        // du lieu cho map
        this._mapData = null;
        // chieu rong cua lop (layer)
        this._layerWidth = 0;
        // chieu cao cua lop (layer)
        this._layerHeight = 0;
        // mang cac Tiles
        this._lastTiles = [];

        /**
         * Cac bitmap duoc su dung nhu bricket cua phan basic.js ma ta da demo
         *
         * @property bitmaps
         * @type Array
         */

        // thuoc tinh bitmaps la mot mang
        this.bitmaps = [];

        /**
         * Xac dinh diem goc de thuc hien hieu ung cuon
         *
         * @property origin
         * @type Point
         */
        
        // khoi tao thuoc tinh origin la mot doi tuong (x, y)
        this.origin = new Point();

        /**
         * Cac dinh danh cho tung tile (tung hinh con)
         *
         * @property flags
         * @type Array
         */

        // cac dinh danh nay la mot mang
        this.flags = [];

        /**
         * So luong hoat canh duoc thuc hien de tao hieu ung cuon chuot
         *
        ...
        /**
         * Thuoc tinh xac dinh tro choi co nam ngang hay khong
         *
        ...
        /**
         * Cho tilemap thuc hien vong lap theo chieu doc
         *
        ...
        // cho khoi tao cac lop
        this._createLayers();
        this.refresh();
    };

    /**
     * Chieu rong cua man hinh tinh bang pixel
     *
     * @property width
     * @type Number
     */

    // phuong thuc xac dinh thuoc tinh cho Object cac thuoc tinh nay cung se duoc Tilemap ke thua
    ...
    /**
     * Chieu cao cua man hinh tinh bang pixel
     *
     * @property height
     * @type Number
     */
    ...
    /**
     * Chieu rong cua mot o tinh bang pixel
     *
     * @property tileWidth
     * @type Number
     */
    ...
    /**
     * Chieu cao cua mot o tinh bang pixel
     *
     * @property tileHeight
     * @type Number
     */
    ...
    /**
     * Sets du lieu cho Tilemap
     *
     * @method setData
     * @param {Number} width Chieu rong cua ban do theo so luong o
     * @param {Number} height Chieu cao cua ban do theo so luon o
     * @param {Array} data Mang mot chieu cho du lieu ban do
     */

    // thuc hien xac dinh chieu cao, chieu rong, va nhan vao data
    Tilemap.prototype.setData = function(width, height, data) {
        // xac dinh chieu rong cua map
    ...
    /**
     * Kiem tra xem tileset da san sang de ket xuat hay chua
     *
     * @method isReady
     * @type Boolean
     * @return {Boolean} Tra ve true neu Tilemap da san sang
     */
    ...
    /**
     * refresh de ve lai toan bo trang web
     *
     * @method refresh
     */
    ...
    /**
     * @method updateTransform: cap nhat cac bien doi
     * @private
     */
...
// Autotile shape number to coordinates of tileset images
    ...
    // Khoi tao doi tuong LevelLoader, phuong thuc initialize se duoc khai bao duoi day
    ...
    LevelLoader.prototype = {
        ...
        // load du lieu: voi ten cua file du lieu va function xu ly
        load : function(name, cb) {
            var tilesets = null;
            var tileset = null;
            var tilesetNames = null;
            var map = null;
            // tao doi tuong loader - pixi core
            var loader = new PIXI.loaders.Loader();
            var self = this;
            function progress(loader, resource) {
                if (resource.name == 'tilesets') {
                    //1. load the map
                    loader.add(name, '/static/imgs/'+name+'.json', {parentResource: resource });
                ...
                if (resource.name == name) {
                    ...
                    tilesetNames.forEach(function(name) {
                        if (name.length>0 && !PIXI.utils.TextureCache[name])
                            loader.add(name, '/static/imgs/'+name +".png", {parentResource: resource });
                    })
                }
            }

            // thuc hien phuong thuc progress cua load voi phuong thuc xu ly progress da duoc dinh nghia o tren
            ...
            if (this.tilesetResource) {
                ...
                loader.add('tilesets', '/static/imgs/Tilesets.json');
            }
